...
class FindTiktokBuzzesWorker
  ...
  def perform(buzz_term_id, frequency = nil, sort_type = 3)
    ...
    play_count_over_1m = Buzz.where("play_count > ? AND created_at >= ? AND user_id = ?", 1_000_000, Time.zone.now.beginning_of_day, buzz_term.user_id).count
    play_count_over_100k = Buzz.where("play_count > ? AND play_count < ? AND created_at >= ? AND user_id = ?", 100_000, 1_000_000, Time.zone.now.beginning_of_day, buzz_term.user_id).count
